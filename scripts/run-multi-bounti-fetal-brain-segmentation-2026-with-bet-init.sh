...
if [[ $# -ne 5 ]] ; then
    ...
    echo "bash /home/perinatal-brain-mri-analysis/run-multi-bounti-fetal-brain-segmentation-2026-with-bet-init.sh [0/1: 0 - no N4 bias correction, 1 - with N4 bias correction] [full_path_to_input_t2w_recon.nii.gz] [full_path_to_input_bet.nii.gz] [full_path_to_folder_for_tmp_processing] [full_path_to_output_label.nii.gz]"
    echo
    echo "------------------------------------------------------------"
    echo
    exit
...
${mirtk}/convert-image ${org_t2} tmp-z.nii.gz

check_dz=$(/bin/MIRTK/build/lib/tools/get-dz tmp-z.nii.gz)

rm  tmp-z.nii.gz


if awk "BEGIN {exit !(($check_dz < 0.49) || ($check_dz > 0.51))}"; then
    echo
    echo "------------------------------------------------------------"
    echo
    echo "ERROR: INPUT RESOLUTION SHOULD BE 0.5mm ... not " $check_dz " mm"
    echo
    echo "------------------------------------------------------------"
    echo
    exit	
else 
    echo " - resolution : " $check_dz " mm"
fi 
...
echo
echo " - bypassing brain extraction - copying input BET mask ..."
echo
...
${mirtk}/transform-image ${in_bet} ${proc}/bet-lab-pad-crop-t2-128.nii.gz -target ${proc}/pad-crop-tr-t2-128.nii.gz -labels 

${mirtk}/threshold-image ${proc}/bet-lab-pad-crop-t2-128.nii.gz ${proc}/bet-lab-pad-crop-t2-128.nii.gz 0.5 > ${proc}/tmp.txt
...
w_lr=${src}/models/atunet_lr_brain_fetal_2lab_best_metric_model.pth
...
cp ${proc}/bet-lab-pad-crop-t2-128.nii.gz .


check_ga=$(/bin/MIRTK/build/lib/tools/guess-ga bet-lab-pad-crop-t2-128.nii.gz)

rm bet-lab-pad-crop-t2-128.nii.gz


#echo  $check_ga 

if [ $check_ga -gt 170 ]; then
	
	cp ${src}/templates/ref-bounti/i.dof  ${proc}/aff-i.dof
else 
	
	cp ${src}/templates/ref-bounti/i-very-early-ga.dof ${proc}/aff-i.dof
fi  

${mirtk}/register ${proc}/tmp-lr-ref.nii.gz ${proc}/lr-lab-pad-masked-crop-t2-128.nii.gz -model Affine -dofin ${proc}/aff-i.dof -dofout ${proc}/aff-d.dof -v 0
...
w_bounti=${src}/models/patch_atunet_new_multi_43_brain_neo_fetal_aff_fix_022026_best_metric_model.pth
