...
if [[ $# -ne 4 ]] ; then
    ...
    echo "bash /home/perinatal-brain-mri-analysis/scripts/run-internal-capsule-neo-brain-segmentation-3t.sh [0/1: 0 - no N4 bias correction, 1 - with N4 bias correction] [full_path_to_input_t2w_recon.nii.gz] [full_path_to_folder_for_tmp_processing] [full_path_to_output_label.nii.gz]"
    echo
    echo "------------------------------------------------------------"
    echo
    exit
...
python3 ${src}/src/run_monai_patch_atunet_segmentation_1case-2026-flip-14-gpu.py 128 14 ${weights_wm} ${proc}/pad-reo-n4-masked-t2-256.nii.gz ${proc}/wm-lab-reo-t2.nii.gz


${mirtk}/transform-image ${proc}/wm-lab-reo-t2.nii.gz ${out_lab} -target ${proc}/org-t2.nii.gz  -dofin_i ${proc}/aff-d.dof -labels
