...
if [[ $# -ne 4 ]] ; then
    ...
    echo "bash /home/perinatal-brain-mri-analysis/run-multi-bounti-neo-brain-segmentation-2026-dhcp.sh [0/1: 0 - no N4 bias correction, 1 - with N4 bias correction] [full_path_to_input_t2w_recon.nii.gz] [full_path_to_folder_for_tmp_processing] [full_path_to_output_label.nii.gz]"
    echo
    echo "------------------------------------------------------------"
    echo
    exit
